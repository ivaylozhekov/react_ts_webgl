...
UUI.Global_Search = (function() {
    ...
    Global_Search.prototype.init = function (callback, full_results_page) {
        ...
        $search.focus(function () {
            $(this).parents('.uui-journalHeader').find('.uui-navigation').hide();
        ...
        $search.blur(function () {
            ...
            setTimeout(function() {
                $(self).val('');
                $(self).parents('.uui-global-search').find('.dropdown-menu').hide();
                $(self).one("transitionend webkitTransitionEnd oTransitionEnd MSTransitionEnd", function () {
                    $(this).parents('.uui-journalHeader').find('.uui-navigation').show();
        ...
        $(window).resize(function () {
            ...
            $resp_search.find('.no-results')
                .css('height', $(window).height() - $resp_search_field.find('.search-journalHeader').height());
        });
    };
    ...
    Global_Search.prototype.show_responsive_results = function(data, query, $resp_search) {
        ...
        if (data.length === 0) {
            ...
            $all_results.hide();
            $resp_search.find('.no-results')
                .css('height', $(window).height() - $resp_search.find('.search-journalHeader').height());
        } else {
            var regexp = new RegExp('(' + query.split(' ').join('|') + ')', 'gi');
            data.forEach(function (item) {
