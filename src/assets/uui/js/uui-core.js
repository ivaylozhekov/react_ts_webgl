    ...
    function Sidebar() {
        ...
        this._timer = function (target) {
            ...
            if (self._menu_timer[target] > 300 && $('.sub', target).is(':hidden')) {
                ...
                $('.arrow', target).removeClass('fa-chevron-right').addClass('fa-chevron-down');
    ...
    Sidebar.prototype.init = function (options) {
        ...
        setTimeout(function () {
            ...
            if ($('.uui-side-bar .sub-menu .sub li').hasClass('active')) {
                ...
                active_item.parent().prev().find('.arrow').removeClass('fa-chevron-right').addClass('fa-chevron-down');
            }
        }, 10);
            ...
            $('.uui-side-bar .sub-menu').click(function (e) {
                if ($(e.target).parents('.sub').length === 0) {
                    ...
                    if ($('.sub', this).is(':hidden')) {
                        $('.arrow').removeClass('fa-chevron-down').addClass('fa-chevron-right');
                        $('.sub').slideUp(300);
                        $('.arrow', this).removeClass('fa-chevron-right').addClass('fa-chevron-down');
                        $('.sub', this).slideDown(300);
                        $(this).addClass('open');
                    }
                    else {
                        $('.arrow', this).removeClass('fa-chevron-down').addClass('fa-chevron-right');
...
UUI.Navigation = (function() {
    ...
    function Navigation() {
        ...
        this._reset_navigation = function () {
            $('.uui-responsive-journalHeader .responsive-menu ul:not(.button-links)').each(function (i, ul) {
                if (i > 0) {
                    $(ul).hide();
                } else {
        ...
        this._set_responsive_navigation_height = function () {
            var $menu_responsive = $('.uui-responsive-journalHeader .responsive-menu');
            var $resp_header_button = $('.uui-responsive-journalHeader .responsive-menu .responsive-journalHeader-button');
            var $resp_global_menu_button = $('.uui-responsive-journalHeader .responsive-menu .responsive-global-menu');
        ...
        this._generate_extended_menu = function() {
            ...
            var header_available_width = $(window).width() - $('.uui-toggle-box').width() -
                $('.breadcrumb-main-nav').width() - $('.brand-logo').outerWidth() - $('.uui-journalHeader-tools').outerWidth() -
                $extended_menu.outerWidth();
            var menu_width = $menu.width();
            if (menu_width >= header_available_width - 20) {
                ...
                if (menu_width + extended_menu_item_width - (header_available_width + 20) < -40) {
                    ...
                    if ($extended_menu.find('ul.dropdown-menu:first li').length === 0) {
    ...
    Navigation.prototype.init = function () {
        ...
        var $resp_header_tools = $('.uui-responsive-journalHeader .uui-journalHeader-tools');
...
UUI.Header_Tools = (function() {
    ...
    Header_Tools.prototype.init = function () {
        $('.uui-journalHeader .dropdown:not(.uui-journalHeader-button):not(.uui-global-search)').hover(
            function () {
        ...
        var $menu_profile_items = $('.uui-journalHeader-tools .uui-profile-menu .profile-links');
        ...
        var $menu_tools_items = $('.uui-journalHeader-tools >li:not(.uui-global-search)');
        $menu_tools_items.each(function () {
            var $item = $(this);
            if ($item.hasClass('dropdown')) {
                var $length_items = $item.length;
                if ($length_items === 1) {
                    var $arrow = $($item.find('.menu-arrow'));
                    setTimeout(function () {
                        var $width_item = $item.width();
                        var $right = ($width_item + 10) / 2 + 6;
        ...
        var $tools = $('.uui-journalHeader-tools');
        $tools.each(function () {
            var $item = $(this);
            var $self = $item.find('>li:not(.uui-global-search):not(:last-child)');
            var $dropdown = $self.find('.dropdown-menu');
            setTimeout(function () {
                $dropdown.css({ 'right': '50%', 'margin-right': -$dropdown.width() / 2 });
                $dropdown.find('.menu-arrow').css('right', '50%');
            }, 10);
        });
    };
